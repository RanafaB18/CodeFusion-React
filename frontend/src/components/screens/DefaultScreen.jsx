...
const DefaultScreen = ({
  ...
  const renderDocs = () => {
    ...
    const docs = tabs.toArray().map((ytext, i) => `<input type="button" index=${i} value="Document ${i}" />`).join('')
    // insert the list of all docs. But the first one is a "create new document" button
    docsDiv.current.innerHTML = '<input type="button" index="new" value="+ Create New Document" />' + docs
    if (tabs.length === 0) {
